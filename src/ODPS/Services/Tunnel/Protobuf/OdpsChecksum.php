...
class OdpsChecksum
{
    ...
    public function update($value)
    {
        ...
        $this->updateBytes($bytes, $offset, $len);

    }

    public function updateBytes($bytes, $offset, $len)
    {
        // $bytes = array_reverse(array_reverse($bytes));
        $bytes = array_values($bytes);
        $localCrc = $this->crc;

        while ($len > 7) {
            $c0 = $bytes[$offset++] ^ $localCrc;

            $c1 = $bytes[$offset++] ^ ($localCrc = $this->shr32($localCrc, 8));

            $c2 = $bytes[$offset++] ^ ($localCrc = $this->shr32($localCrc, 8));;

            $c3 = $bytes[$offset++] ^ ($localCrc = $this->shr32($localCrc, 8));;


            $localCrc = ($this->T8_7[$c0 & 0xff] ^ $this->T8_6[$c1 & 0xff])
                ^ ($this->T8_5[$c2 & 0xff] ^ $this->T8_4[$c3 & 0xff]);


            $localCrc ^= ($this->T8_3[$bytes[$offset++] & 0xff] ^ $this->T8_2[$bytes[$offset++] & 0xff])
                ^ ($this->T8_1[$bytes[$offset++] & 0xff] ^ $this->T8_0[$bytes[$offset++] & 0xff]);
            $len -= 8;
        }

        while ($len > 0) {

            $idx = ($localCrc ^ $bytes[$offset++]);

            $localCrc = ($this->shr32($localCrc, 8)) ^ $this->T8_0[($idx) & 0xff];
            $len--;
        }
