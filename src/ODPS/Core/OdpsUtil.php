...
class OdpsUtil
{
    const ODPS_CONTENT = 'content';
    const ODPS_LENGTH = 'length';
    const ODPS_HEADERS = 'headers';
    const ODPS_MAX_OBJECT_GROUP_VALUE = 1000;
    const ODPS_MAX_PART_SIZE = 5368709120; // 5GB
    const ODPS_MID_PART_SIZE = 10485760; // 10MB
    const ODPS_MIN_PART_SIZE = 102400; // 100KB
     ...
     */
    public static function toQueryString($queryParameters = array(), $options)
    {
        $temp = array();
        $arrayKeys = array_keys($queryParameters);

        if (isset($options[OdpsClient::ODPS_SUB_RESOURCE])) {
            $arrayKeys[] = $options[OdpsClient::ODPS_SUB_RESOURCE];
        }

        usort($arrayKeys, 'strnatcasecmp');
...
BBB;
    ...
    {
        $content_md5 = "";
        if (($to_pos - $from_pos) > self::ODPS_MAX_PART_SIZE) {
            return $content_md5;
        }
    ...
    public static function createDeleteObjectsXmlBody($objects, $quiet)
    {
        ...
        foreach ($objects as $object) {
            ...
            $object = OdpsUtil::sReplace($object);
    ...
    public static function zigZagVal($val)
    {
        return ($val << 1) ^ ($val >> 63);
    }

    public static function unZigZagVal($val)
    {
        return static::shr($val, 1) ^ -($val & 1);
    }

    public static function shr($x, $bits)
    {
