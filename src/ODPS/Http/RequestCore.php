...
class RequestCore
{
     ...
     */
    public $request_class = 'ODPS\Http\RequestCore';
    /**
     * The default class to use for HTTP Responses (defaults to <ResponseCore>).
     */
    public $response_class = 'ODPS\Http\ResponseCore';
    /**
     * Default useragent string to use.
     */
    public $useragent = 'ODPS PHP SDK/1.0.0';
    /**
     ...
     */
    public function __construct($url = null, $proxy = null, $helpers = null, $bySocket = false)
    {
    ...
    public function openRequestSocket()
    {
        $parse = $this->_parseUrl($this->request_url);
        $content = $this->_buildRequestContent($parse, true);
        $socket = null;
        if ($this->proxy) {
    ...
    /**
     * Parse sockst http requst result
     *
    ...
    public function parseResponse($response)
    {
        ...
        $header = $this->parseHttpSocketHeader($headerContent);
        $response = trim(stristr($response, "\r\n\r\n"), "\r\n");

        return new ResponseCore($header, $this->_checkChunk($response, $header), isset($header["http_code"]) ? $header["http_code"] : 0);
    }

    /**
     * Transform header string to an array
     *
     * @param  string $str headerString。
     * @return array headers。
     */
    public static function parseHttpSocketHeader($str)
    {
        $slice = explode("\r\n", $str);
        $headers = array();
        foreach ($slice as $v) {
    ...
    private function _socketRequest()
    {
        $socket = $this->openRequestSocket();
        $response = '';
    ...
    /**
     * Parse url to an object which contains:host、schema、path、query...。
     *
     * @param  string $url
     * @throws Exception
    ...
    /**
     * Build request head for socket call
     *
     * @param  array $parse
     * @param  string $method 。
     * @return string
     */
    private function _buildRequestContent(&$parse, $withoutCloseConnection)
    {
        $write = $this->method . " " . $this->request_url . " HTTP/1.1\r\n";
    ...
    /**
     * Veriry whether server returns Chrunk, if it is yes returns chrunk length from body, otherwise returns all content
     *
     * @param string $data body
     * @param array $header header
     * @param string $rn chunk
     *
     * @return string  。
     */
